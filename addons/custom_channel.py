...
class CustomOutput(TeleBot, OutputChannel):
    """custom Output channel for Telegram."""

    # skipcq: PYL-W0236
    @classmethod
    def name(cls) -> Text:
        return "custom_telegram"
    ...
    async def send_text_with_buttons(
        ...
        **kwargs: Any,
        ...
        elif button_type == "vertical_url":
            reply_markup = InlineKeyboardMarkup()
            [
                reply_markup.row(
                    InlineKeyboardButton(s["title"], url=s["url"])
                )
                for s in buttons
            ]
...
class CustomInput(InputChannel):
    ...
    def blueprint(
        ...
        custom_telegram_webhook = Blueprint("custom_telegram_webhook", __name__)
        ...
        @custom_telegram_webhook.route("/set_webhook", methods=["GET", "POST"])
        ...
        @custom_telegram_webhook.route("/webhook", methods=["GET", "POST"])
    ...
    def get_output_channel(self) -> CustomOutput:
        """Loads the custom telegram channel."""
        channel = CustomOutput(self.access_token)
